...
function NavItem({
  ...
  return (
    <NavLink
      ...
      className={({ isActive }) =>
        clsx(
          'group flex items-center rounded-lg mb-0.5 transition-all',
      ...
      {({ isActive }) => (
        <>
          <item.icon
            ...
            className={clsx(
              'shrink-0',
              isActive ? 'text-brand-400' : 'text-surface-600 group-hover:text-surface-400',
            )}
          />
          {!collapsed && (
            <>
              <span className={clsx('font-medium text-sm flex-1', isActive && 'text-brand-300')}>
                {item.label}
              </span>
              {isActive && <ChevronRight size={11} className="text-brand-600" />}
            </>
          )}
        </>
      )}
...
// ── Hierarchy explorer (hidden when collapsed) ─────────────────────────────
function HierarchyExplorer() {
  ...
  const pick = <T extends { id: number; name: string }>(
    ...
    <div className="space-y-0.5">
      {loading && <div className="px-3 py-1 text-[10px] text-surface-600 animate-pulse">Loading…</div>}
      {items.map(item => {
        ...
        return (
          <button
            key={item.id}
            onClick={() => {
              isActive ? onSelect(null) : (onSelect(item.id), pushBreadcrumb({ level, id: item.id, label: item.name }))
            }}
            className={clsx(
          ...
          {selectedPlantId && (
            <div className={indent}>
              ...
              {selectedShedId && (() => {
                ...
                const isSingleSameName = sections.length === 1 && sections[0].name === selectedShedObj?.name
                if (isSingleSameName && sections[0]) {
                  if (!selectedSectionId) selectSection(sections[0].id)
                  return (
                    <div className={indent}>
...
export default function Sidebar({ collapsed = false, onToggleCollapse, onClose }: SidebarProps) {
  return (
    <aside
      className={clsx(
        'bg-surface-900 border-r border-surface-800 flex flex-col h-screen sticky top-0 overflow-hidden transition-all duration-200',
        collapsed ? 'w-[52px]' : 'w-56',
      )}
    >
      {/* ── Logo ──────────────────────────────────────────────── */}
      <div className={clsx(
        'border-b border-surface-800 flex items-center shrink-0',
        collapsed ? 'justify-center h-12' : 'px-4 py-3.5',
      )}>
        {collapsed ? (
          /* Collapsed: icon only */
          ...
          /* Expanded: logo + wordmark + optional close */
      ...
      {/* ── Navigation ────────────────────────────────────────── */}
      <nav className={clsx('pt-3 shrink-0', collapsed ? 'px-1.5' : 'px-2')}>
        {!collapsed && (
      ...
      {/* Spacer when collapsed so toggle sits at bottom */}
