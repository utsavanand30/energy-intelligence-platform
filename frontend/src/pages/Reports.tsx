...
import { fmtKwh, fmtPf } from '../utils/formatters'
import { exportPolycabMonthlyReport, exportGenericReport } from '../utils/excelExport'
import {
  FileText, Download, RefreshCw, Plus, X,
  GripVertical, Check, BarChart3, Settings, FileSpreadsheet, Star,
} from 'lucide-react'
...
// ── Templates (including Polycab default) ───────────────────────────────────
const TEMPLATES = [
  {
    id: 'polycab',
    name: 'Polycab Monthly Report',
    description: 'Process-wise & Machine-wise daily readings with monthly total — matches reference Excel format',
    columns: ['date', 'section_name', 'machine_name', 'meter_identification', 'consumption_kwh', 'avg_kw', 'avg_pf'],
    icon: '⭐',
    isDefault: true,
    excelFormat: 'polycab',
  },
  {
    id: 'summary',
    name: 'Daily Summary',
    description: 'Essential consumption per machine',
  ...
  },
  {
    ...
    description: 'Full register data with opening/closing kWh',
  ...
  },
  {
    ...
    description: 'Power quality metrics by machine',
  ...
  },
  {
    id: 'meter',
    name: 'Meter Register',
    description: 'Meter-level opening / closing register',
...
export default function Reports() {
  ...
  const [activeExcelFormat, setActiveExcelFormat] = useState<'generic' | 'polycab'>('generic')
  const [savedReports, setSavedReports]   = useState<{
    id: string; name: string; columns: string[]; createdAt: string; excelFormat: 'generic' | 'polycab'
  }[]>([])
  ...
  }, [activeTab, selectedPlantId, selectedShedId, selectedSectionId, fromDate, toDate]) // eslint-disable-line

  // ── Export ────────────────────────────────────────────────────────────────
  const handleExcelExport = (fmt: 'generic' | 'polycab' = activeExcelFormat) => {
    if (rows.length === 0) return
    if (fmt === 'polycab') {
      exportPolycabMonthlyReport(rows, fromDate, toDate)
    } else {
      const cols = ALL_COLUMNS.filter(c => activeColumns.includes(c.key))
      exportGenericReport(rows, cols, fromDate, toDate)
    }
  }
  ...
  const exportCsv = () => {
    ...
    const csvRows = [
      headers.join(','),
      ...rows.map(r => cols.map(c => formatCell(r, c.key).replace(',', ';')).join(',')),
    ]
    const blob = new Blob([csvRows.join('\n')], { type: 'text/csv' })
    const url = URL.createObjectURL(blob)
    const a = document.createElement('a')
    a.href = url
    a.download = `energy_report_${fromDate}_${toDate}.csv`
    a.click()
  ...
  const applyTemplate = (tmpl: typeof TEMPLATES[0]) => {
    setActiveColumns(tmpl.columns)
    setActiveExcelFormat(tmpl.excelFormat as 'generic' | 'polycab')
    setActiveTab('daily')
    load()
  }
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      {/* Controls bar */}
      <div className="px-4 sm:px-5 py-2.5 border-b border-surface-800 bg-surface-950/60 flex flex-wrap items-end gap-3">
        ...
        <div className="flex items-end gap-2 flex-wrap ml-auto">
          ...
          <div className="flex gap-1">
            <button
              onClick={() => handleExcelExport('polycab')}
              disabled={rows.length === 0}
              title="Download Polycab Monthly Excel (matches reference format)"
              className="btn-secondary text-xs gap-1.5 py-1.5 disabled:opacity-40 border-emerald-700/40 hover:border-emerald-600 text-emerald-400 hover:text-emerald-300"
            >
              <FileSpreadsheet size={13} />
              <span className="hidden sm:inline">Polycab Excel</span>
              <span className="sm:hidden">XLS</span>
            </button>
            <button
              onClick={() => handleExcelExport('generic')}
              disabled={rows.length === 0}
              title="Download generic Excel report"
              className="btn-secondary text-xs gap-1.5 py-1.5 disabled:opacity-40"
            >
              <Download size={12} />
      ...
      {/* Tabs */}
      <div className="px-4 sm:px-5 bg-surface-950/40">
            ...
            {loading ? (
              ...
              <div className="card p-12 text-center">
                ...
                <p className="text-xs text-surface-600 mt-1">
                  Select a date range that overlaps with the historical seed window
                </p>
              </div>
            ) : (
                ...
                <div className="card-header flex-wrap gap-2">
                  <div>
                    ...
                    <span className="text-xs text-surface-500 ml-2">
                      · {rows.length} rows · {activeColumns.length} columns
                    </span>
                  </div>
                        ...
                        <tr key={`subtotal-${sectionName}`} className="border-b border-surface-700 bg-surface-800/20">
                          {activeColumns.map(key => (
                            <td key={key} className="px-3 py-1.5 text-xs font-bold">
                              {key === 'section_name'    ? <span className="text-surface-400">{sectionName} subtotal</span>
                      ...
                      <tr className="bg-surface-800/60 border-t border-surface-700">
                        {activeColumns.map(key => (
                          <td key={key} className="px-3 py-2 text-xs font-bold">
                            {key === 'machine_name'    ? <span className="text-surface-300">GRAND TOTAL</span>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
              ...
              <div className="card">
                <div className="card-header">
                  ...
                  <span className="text-xs text-surface-500">Click to toggle</span>
                </div>
                          ...
                          return (
                            <button
                              ...
                              className={clsx(
                                ...
                                isActive ? 'bg-brand-600/20 text-brand-300 border border-brand-600/30'
              ...
              {/* Middle: Active columns + Excel format selector */}
              <div className="space-y-4">
                <div className="card">
                  ...
                  <div className="p-3 space-y-1.5">
                    {activeColumns.map((key, idx) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {!col?.required && (
                            <button onClick={() => toggleColumn(key)} className="text-surface-600 hover:text-red-400 transition-colors">
                              <X size={12} />
                            </button>
                          )}
                        </div>
                    ...
                    {activeColumns.length === 0 && (
                      <div className="py-8 text-center text-xs text-surface-500">Add columns from the left panel</div>
                    )}
                  </div>
                </div>
                ...
                <div className="card p-3 space-y-2">
                  <div className="text-xs font-semibold text-surface-300 mb-2">Excel Export Format</div>
                  {[
                    { value: 'polycab',  label: 'Polycab Monthly Format', desc: 'Process × Day matrix with monthly total', icon: '⭐' },
                    { value: 'generic',  label: 'Generic Flat Table',      desc: 'Simple row-per-reading format',           icon: '📋' },
                  ].map(opt => (
                    <label
                      key={opt.value}
                      className={clsx(
                        'flex items-start gap-2.5 p-2.5 rounded-lg cursor-pointer border transition-all',
                        activeExcelFormat === opt.value
                    ...
                    >
                      <input
                        ...
                        name="excelFmt"
                        value={opt.value}
                        checked={activeExcelFormat === opt.value}
                        onChange={() => setActiveExcelFormat(opt.value as 'generic' | 'polycab')}
                        className="mt-0.5"
                      />
                      <div>
                        <div className="text-xs font-medium text-surface-200">{opt.icon} {opt.label}</div>
                    ...
                    <button
                      ...
                      className={clsx(
                        ...
                        t.isDefault
                          ...
                          : 'border-surface-700 bg-surface-800 hover:bg-surface-700 hover:border-surface-600',
                      )}
                    >
                      <div className="flex items-center gap-2 mb-1">
                        <span>{t.icon}</span>
                        <span className={clsx('text-sm font-medium group-hover:text-white transition-colors',
                          t.isDefault ? 'text-emerald-300' : 'text-surface-200')}>
                          {t.name}
                        </span>
                        {t.isDefault && (
                          <span className="ml-auto text-[9px] bg-emerald-900/50 text-emerald-400 px-1.5 py-0.5 rounded border border-emerald-700/40">
                            DEFAULT
                          </span>
                        )}
                      </div>
          ...
          <div className="p-4 sm:p-5">
            {savedReports.length === 0 ? (
              <div className="card p-12 text-center">
                ...
                <p className="text-sm text-surface-400">No saved templates yet</p>
                <p className="text-xs text-surface-600 mt-1">
                ...
                {savedReports.map(r => (
                  <div key={r.id} className="card p-4 hover:border-surface-700 transition-colors">
                    ...
                    <div className="text-[10px] text-surface-500 mb-1">
                      Created {r.createdAt} · {r.columns.length} columns
                    </div>
                    <div className="text-[9px] text-surface-600 mb-3">
                      Format: {r.excelFormat === 'polycab' ? '⭐ Polycab Monthly' : '📋 Generic'}
                    </div>
                    ...
                    <div className="flex gap-2">
                      <button
                        onClick={() => {
                          ...
                          setActiveExcelFormat(r.excelFormat)
                          setActiveTab('daily')
                          load()
                        }}
                      ...
                      <button
                        onClick={() => {
                          setActiveColumns(r.columns)
                          setActiveExcelFormat(r.excelFormat)
                          setTimeout(() => handleExcelExport(r.excelFormat), 100)
                        }}
      ...
      {showSaveDialog && (
        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center backdrop-blur-sm p-4">
          <div className="bg-surface-900 border border-surface-700 rounded-xl p-6 w-full max-w-sm shadow-2xl">
            <h3 className="text-base font-semibold text-white mb-1">Save Report Template</h3>
            <p className="text-xs text-surface-500 mb-4">
              {activeColumns.length} columns · {activeExcelFormat === 'polycab' ? 'Polycab Excel format' : 'Generic Excel format'}
            </p>
            <input
